...
# Delete the last two lines of that file to restore it.
# Usage: traj2movie.sh trajectory.xyz
set -e

nImages=$(grep -c 'Properties' "$1")
NAME="${1%.xyz}"
echo "Processing ${nImages} images."

if [[ ! -f "${NAME}.vmd" ]]; then
   read -rp "VMD will now open, save the view as ${NAME}.vmd, then exit. Press enter to continue"
   vmd "$1"
fi
